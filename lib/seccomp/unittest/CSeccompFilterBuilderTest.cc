...
BOOST_AUTO_TEST_CASE(testDegradedModeAttestationMarkerRouteLandlock) {
    ...
    // the marker.
    BOOST_TEST_REQUIRE(
        degradedModeAttestationMarker(ESystemCallFilterInstallOutcome::E_MechanismUnavailable, "landlock")
            .empty());
    BOOST_TEST_REQUIRE(
        degradedModeAttestationMarker(ESystemCallFilterInstallOutcome::E_PrivilegeRestrictionFailed, "landlock")
            .empty());
    BOOST_TEST_REQUIRE(
        degradedModeAttestationMarker(ESystemCallFilterInstallOutcome::E_FilterInstallFailed, "landlock")
            .empty());
}
...
BOOST_AUTO_TEST_CASE(testApplyInProcessSeccompFilterFailedInstallEmptyMarkerRegardlessOfRoute) {
    ...
    for (const auto outcome : failureModes) {
        for (const char* route : routes) {
            const auto result =
                applyInProcessSeccompFilter(false, true, [outcome] { return outcome; }, route);
            BOOST_REQUIRE_EQUAL(true, result.s_Attempted);
            BOOST_TEST_REQUIRE(result.s_AttestationMarker.empty());
        }
    }
