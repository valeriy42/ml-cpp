...
using CMultimodalPrior = CPriorTestInterfaceMixin<maths::common::CMultimodalPrior>;
using CNormalMeanPrecConjugate = CPriorTestInterfaceMixin<maths::common::CNormalMeanPrecConjugate>;
using COneOfNPrior = CPriorTestInterfaceMixin<maths::common::COneOfNPrior>;
using CPoissonMeanConjugate = CPriorTestInterfaceMixin<maths::common::CPoissonMeanConjugate>;
...
BOOST_AUTO_TEST_CASE(testFilter) {
    TPriorPtrVec models;
    models.push_back(TPriorPtr(maths::common::CGammaRateConjugate::nonInformativePrior(E_ContinuousData)
                                   .clone()));
    models.push_back(TPriorPtr(maths::common::CLogNormalMeanPrecConjugate::nonInformativePrior(E_ContinuousData)
                                   .clone()));
    models.push_back(TPriorPtr(maths::common::CNormalMeanPrecConjugate::nonInformativePrior(E_ContinuousData)
                                   .clone()));
    models.push_back(TPriorPtr(
        maths::common::CPoissonMeanConjugate::nonInformativePrior().clone()));
...
BOOST_AUTO_TEST_CASE(testMultipleUpdate) {
    ...
    COneOfNPrior filter1(maths::common::COneOfNPrior(clone(models), E_ContinuousData));
    COneOfNPrior filter2(maths::common::COneOfNPrior(clone(models), E_ContinuousData));
    ...
    TEqual equal(maths::common::CToleranceTypes::E_AbsoluteTolerance, 1e-3);
    ...
    const maths::common::CPoissonMeanConjugate* poisson1 =
        dynamic_cast<const maths::common::CPoissonMeanConjugate*>(models1[0]);
    const maths::common::CPoissonMeanConjugate* poisson2 =
        dynamic_cast<const maths::common::CPoissonMeanConjugate*>(models2[0]);
    BOOST_TEST_REQUIRE(poisson1);
    BOOST_TEST_REQUIRE(poisson2);
    BOOST_TEST_REQUIRE(poisson1->equalTolerance(*poisson2, equal));

    const maths::common::CNormalMeanPrecConjugate* normal1 =
        dynamic_cast<const maths::common::CNormalMeanPrecConjugate*>(models1[1]);
    const maths::common::CNormalMeanPrecConjugate* normal2 =
        dynamic_cast<const maths::common::CNormalMeanPrecConjugate*>(models2[1]);
...
BOOST_AUTO_TEST_CASE(testModels) {
...

    {
        ...
        const maths::common::CPoissonMeanConjugate* poissonModel =
            dynamic_cast<const maths::common::CPoissonMeanConjugate*>(posteriorModels[0]);
        const maths::common::CNormalMeanPrecConjugate* normalModel =
            dynamic_cast<const maths::common::CNormalMeanPrecConjugate*>(posteriorModels[1]);
...
BOOST_AUTO_TEST_CASE(testModelSelection) {
...

    {
        ...
        double poissonExpectedLogWeight = -maths::common::CTools::differentialEntropy(poisson);
        double normalExpectedLogWeight = -maths::common::CTools::differentialEntropy(normal);
...

    {
        ...
        double logNormalExpectedLogWeight =
            -maths::common::CTools::differentialEntropy(logNormal);
        double normalExpectedLogWeight = -maths::common::CTools::differentialEntropy(normal);
    ...
    }
    {
        ...
        maths::common::COneOfNPrior::TPriorPtrVec mode;
        mode.push_back(TPriorPtr(normal.clone()));
        models.push_back(TPriorPtr(new maths::common::CMultimodalPrior(
            maths_t::E_ContinuousData, clusterer,
            maths::common::COneOfNPrior(mode, maths_t::E_ContinuousData))));
        COneOfNPrior filter(maths::common::COneOfNPrior(clone(models), E_ContinuousData));
...
BOOST_AUTO_TEST_CASE(testMarginalLikelihood) {
    ...
    for (std::size_t t = 0; t < std::size(dataTypes); ++t) {
        ...
        COneOfNPrior filter(maths::common::COneOfNPrior(clone(models), dataTypes[t]));
    ...
    COneOfNPrior filter(maths::common::COneOfNPrior(models, E_ContinuousData));
...

    {
        ...
        for (std::size_t i = 0; i < std::size(locations); ++i) {
            for (std::size_t j = 0; j < std::size(squareScales); ++j) {
                ...
                BOOST_TEST_REQUIRE(maths::common::CBasicStatistics::mean(relativeError) < 0.02);
            }
        }
    }
}
...

    {
        ...
        for (std::size_t i = 0; i < std::size(means); ++i) {
            for (std::size_t j = 0; j < std::size(variances); ++j) {
                ...
                double mode;
                double fmode;
                maths::common::CCompositeFunctions::CExp<maths::common::CPrior::CLogMarginalLikelihood> likelihood(
                    filter);
                double a = means[i] - 2.0 * std::sqrt(variances[j]);
                double b = means[i] + 2.0 * std::sqrt(variances[j]);
                maths::common::CSolvers::maximize(a, b, likelihood(a),
                                                  likelihood(b), likelihood,
                                                  0.0, iterations, mode, fmode);
...

    {
        ...
        for (std::size_t i = 0; i < std::size(means); ++i) {
            for (std::size_t j = 0; j < std::size(variances); ++j) {
                ...
                LOG_DEBUG(<< "relativeError = "
                          << maths::common::CBasicStatistics::mean(relativeError));
                BOOST_TEST_REQUIRE(maths::common::CBasicStatistics::mean(relativeError) < 2e-3);
            }
        }
    }

    {
        ...
        for (std::size_t i = 0; i < std::size(shapes); ++i) {
            for (std::size_t j = 0; j < std::size(scales); ++j) {
                ...
                LOG_DEBUG(<< "relativeError = "
                          << maths::common::CBasicStatistics::mean(relativeError));
                BOOST_TEST_REQUIRE(maths::common::CBasicStatistics::mean(relativeError) < 3e-3);
            }
        }
    }
}
...
BOOST_AUTO_TEST_CASE(testProbabilityOfLessLikelySamples) {
    ...
    COneOfNPrior filter(maths::common::COneOfNPrior(clone(initialModels), E_ContinuousData));
...
BOOST_AUTO_TEST_CASE(testPersist) {
    ...
    maths::common::COneOfNPrior origFilter(clone(models), E_IntegerData);
    for (std::size_t i = 0; i < samples.size(); ++i) {
    ...
    maths::common::COneOfNPrior restoredFilter(params, traverser);
...
BOOST_AUTO_TEST_CASE(testBadValues) {
    ...
    TDoubleVec samples;
    rng.generateLogNormalSamples(std::log(2.0), 1.0, 100, samples);
