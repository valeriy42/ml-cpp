...
using TClusterVec = maths::common::CXMeansOnline1d::TClusterVec;
...
BOOST_AUTO_TEST_CASE(testCluster) {
    ...
    double expectedCount = maths::common::CBasicStatistics::count(moments);
    double expectedCentre = maths::common::CBasicStatistics::mean(moments);
    double expectedSpread = std::sqrt(maths::common::CBasicStatistics::variance(moments));
    ...
    maths::common::CBasicStatistics::SSampleMeanVar<double>::TAccumulator percentileError;
    ...
    LOG_DEBUG(<< "mean error = " << maths::common::CBasicStatistics::mean(percentileError));
    ...
    double sampleCentre = maths::common::CBasicStatistics::mean(sampleMoments);
    double sampleSpread = std::sqrt(maths::common::CBasicStatistics::variance(sampleMoments));
...
BOOST_AUTO_TEST_CASE(testMixtureOfGaussians) {
    ...
    maths::common::CXMeansOnline1d::TSizeDoublePr2Vec dummy;
    {
        LOG_DEBUG(<< "*** Test 1 ***");
        ...
        for (unsigned int i = 0; i < 50; ++i) {
            ...
            for (std::size_t j = 0; j < clusters.size(); ++j) {
                ...
                BOOST_REQUIRE_CLOSE_ABSOLUTE(
                    std::sqrt(maths::common::CBasicStatistics::variance(expectedClusters[j])),
                    clusters[j].spread(), 0.4);
                meanError += std::fabs(
                    clusters[j].centre() -
                    maths::common::CBasicStatistics::mean(expectedClusters[j]));
                spreadError += std::fabs(clusters[j].spread() -
                                         std::sqrt(maths::common::CBasicStatistics::variance(
                                             expectedClusters[j])));
            }
        }
    ...
    //   * Cluster 2 = N(15, 2), 1600 points
    {
        ...
        BOOST_REQUIRE_CLOSE_ABSOLUTE(
            std::sqrt(maths::common::CBasicStatistics::variance(expectedClusters)),
    ...
    for (unsigned int i = 0; i < 50; ++i) {
        ...
        for (std::size_t j = 0; j < clusters.size(); ++j) {
            BOOST_REQUIRE_CLOSE_ABSOLUTE(
                maths::common::CBasicStatistics::mean(expectedClusters[j]),
                clusters[j].centre(),
                0.03 * std::max(maths::common::CBasicStatistics::mean(expectedClusters[j]),
                                clusters[j].centre()));
            BOOST_REQUIRE_CLOSE_ABSOLUTE(
                std::sqrt(maths::common::CBasicStatistics::variance(expectedClusters[j])),
                clusters[j].spread(),
                0.5 * std::max(std::sqrt(maths::common::CBasicStatistics::variance(
                                   expectedClusters[j])),
                               clusters[j].spread()));
            meanError += std::fabs(
                clusters[j].centre() -
    ...
    for (unsigned int i = 0; i < 50; ++i) {
        ...
        for (std::size_t j = 0; j < clusters.size(); ++j) {
            BOOST_REQUIRE_CLOSE_ABSOLUTE(
                maths::common::CBasicStatistics::mean(expectedClusters[j]),
                clusters[j].centre(),
                0.01 * std::max(maths::common::CBasicStatistics::mean(expectedClusters[j]),
                                clusters[j].centre()));
            BOOST_REQUIRE_CLOSE_ABSOLUTE(
                std::sqrt(maths::common::CBasicStatistics::variance(expectedClusters[j])),
                clusters[j].spread(),
                0.03 * std::max(std::sqrt(maths::common::CBasicStatistics::variance(
                                    expectedClusters[j])),
                                clusters[j].spread()));
            meanError += std::fabs(
                clusters[j].centre() -
...
BOOST_AUTO_TEST_CASE(testManyClusters) {
    ...
    maths::common::CXMeansOnline1d clusterer(
        maths_t::E_IntegerData, maths::common::CAvailableModeDistributions::ALL,
        maths_t::E_ClustersFractionWeight,
        0.001, // decay rate
        0.01,  // mode fraction
        2);    // mode count
    ...
    auto addSamplesToClusterer = [&](const std::size_t& limit,
        ...
        for (std::size_t j = 0; j < clusters.size(); ++j) {
            ...
            BOOST_REQUIRE_CLOSE_ABSOLUTE(
                std::sqrt(maths::common::CBasicStatistics::variance(expectedClusters[j])),
                clusters[j].spread(),
                0.04 * std::max(std::sqrt(maths::common::CBasicStatistics::variance(
                                    expectedClusters[j])),
                                clusters[j].spread()));
        }
    };
...
BOOST_AUTO_TEST_CASE(testLargeHistory) {
    ...
    maths::common::CXMeansOnline1d reference(
        maths_t::E_ContinuousData, maths::common::CAvailableModeDistributions::ALL,
        maths_t::E_ClustersFractionWeight,
        0.001, // decay rate
...
BOOST_AUTO_TEST_CASE(testPruneEmptyCluster) {
    ...
    maths::common::CXMeansOnline1d::CCluster cluster1(clusterer);
    ...
    maths::common::CXMeansOnline1d::CCluster cluster2(clusterer);
