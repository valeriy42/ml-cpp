...
// Linux-only mechanism-probe integration test for PR C of
// docs/projects/mlcpp-sandbox2-pr2873 (design.md gates V4, V7, and the
// policy half of V5). Builds a real filesystem/network policy via
...
//
// NOT YET RUN: this test has not been executed on a real Linux host in
// this session (macOS host has no Sandbox2/Linux toolchain). It has been
// reviewed against the Sandbox2 PolicyBuilder API as used by the
// PR-A-verified CSandboxForkserverSmokeTest_Linux, but needs a devbox or
// Buildkite pass before its gates (V4/V7/V16) can be marked verified - see
// elastic-workspace-3b59.4 session notes.
...
BOOST_AUTO_TEST_CASE(testMinimizedPolicyEnforcesEveryMechanism) {
    ...
    // childRoot path once the sandbox has exited. This IS the V4 "allowed
    // IPC access" proof, not a separate assertion: if the mount/policy were
