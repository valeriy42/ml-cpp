...
// Purpose-built allowlisted payload for PR C's mechanism probe (design.md
// Sandbox2 clean rebuild plan, gates V4/V7/V16). Runs *inside* the sandbox
// under the policy built by buildPytorchInferenceFilesystemPolicy and prints
...
//! genuinely shared, so this doubles as the V4 "allowed IPC access" proof
//! and as this probe's only result channel (no stdout capture plumbing
//! exists yet; that lands with CSandboxedProcessSpawner in PR D).
int g_ResultsFd = -1;
...
void report(const char* mechanism, const char* outcome, const std::string& detail = "") {
    ...
    std::fflush(stdout);
    if (g_ResultsFd >= 0) {
        std::string line{std::string("mechanism=") + mechanism + " outcome=" + outcome + " detail=" +
                         detail + "\n"};
...
int main(int argc, char** argv) {
    ...
    g_ResultsFd = ::open((ipcDir + "/results.txt").c_str(), O_CREAT | O_WRONLY | O_TRUNC, 0600);
    ...
    // Allowed IPC access (V4 positive control): write then read back a file
    ...
    // Denied host read (V4 negative control): /etc/shadow must not be
    ...
    // Private tmpfs (V4): the mapped /tmp must be writable but is a private
    // tmpfs, never the host's shared /tmp - this re-proves it is not shared
    // by checking it is actually writable from inside the sandbox.
    ...
    if (tmpFd >= 0) {
        ...
        report("private_tmpfs_write", "allowed");
    } else {
        report("private_tmpfs_write", "denied", std::strerror(errno));
    }

    // Mount enumeration (V16 conformance): /etc must list only the
    ...
    // External egress denial (V7 negative control): an outbound connect to
    ...
    if (egressSocket >= 0) {
        ...
        const int rc = ::connect(egressSocket, reinterpret_cast<sockaddr*>(&addr), sizeof(addr));
        report("external_egress", rc == 0 ? "allowed" : "denied", std::strerror(errno));
        ::close(egressSocket);
    } else {
        report("external_egress", "denied", std::strerror(errno));
    }

    // Local operation success (V7 positive control): loopback must remain
    ...
    if (loopbackSocket >= 0) {
        ...
        const int rc = ::connect(loopbackSocket, reinterpret_cast<sockaddr*>(&addr), sizeof(addr));
        const bool loopbackReachable = rc == 0 || errno == ECONNREFUSED;
        report("loopback_reachable", loopbackReachable ? "ok" : "broken", std::strerror(errno));
