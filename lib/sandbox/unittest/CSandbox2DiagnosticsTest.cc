...
BOOST_AUTO_TEST_CASE(testNoConfinementMessageExplainsAndTellsTheOperatorWhatToDo) {
    ...
    const std::string tooOldMessage{
        ml::sandbox::noConfinementMessage(tooOld, "/usr/share/elasticsearch/bin/pytorch_inference")};
    ...
    const std::string blockedMessage{
        ml::sandbox::noConfinementMessage(blocked, "/usr/share/elasticsearch/bin/pytorch_inference")};
