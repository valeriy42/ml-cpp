...
// contract (design.md Sandbox2 clean rebuild plan, PR C, gate V16). This
// suite is deliberately platform-portable - it only needs realpath()/mkdir()
// /symlink(), not Sandbox2 itself - so it runs on every ml-cpp CI platform,
// not just Linux.
...
public:
    explicit CTempChildIpcFixture(const std::string& childId) : m_ChildId(childId) {
...
BOOST_AUTO_TEST_CASE(testRejectsRelativePath) {
    ...
    BOOST_REQUIRE(result.s_Rejected[0].s_Reason == ml::sandbox::EChildIpcPathRejection::E_NotAbsolute);
}

BOOST_AUTO_TEST_CASE(testRejectsRootLevelPath) {
    CTempChildIpcFixture fixture{"child-3"};
    const ml::sandbox::SChildIpcValidationResult result{
        ml::sandbox::validateChildIpcLaunchSpec(fixture.canonicalTrustedBase(), {"--input=/input.fifo"})};
...
BOOST_AUTO_TEST_CASE(testRejectsDotDotEscape) {
    ...
    BOOST_TEST_REQUIRE(result.s_Ok == false);
    BOOST_REQUIRE_EQUAL(result.s_Rejected.size(), 1);
    BOOST_REQUIRE(result.s_Rejected[0].s_Reason == ml::sandbox::EChildIpcPathRejection::E_ContainsDotDot);
}
...
BOOST_AUTO_TEST_CASE(testRejectsWrongDepthDirectChildOfTrustedBase) {
    ...
    // this was the exact wording bug the adversarial review (M1) caught in
    // an earlier draft of this contract.
    const std::string tooShallow{fixture.canonicalTrustedBase() + "/input.fifo"};
    BOOST_TEST_REQUIRE(::mkdir((fixture.canonicalTrustedBase() + "/direct-child-dir").c_str(), 0700) == 0);
    ...
    BOOST_REQUIRE_EQUAL(result.s_Rejected.size(), 1);
    BOOST_REQUIRE(result.s_Rejected[0].s_Reason == ml::sandbox::EChildIpcPathRejection::E_RootLevelPath ||
                  result.s_Rejected[0].s_Reason == ml::sandbox::EChildIpcPathRejection::E_WrongDepth);

    ::rmdir((fixture.canonicalTrustedBase() + "/direct-child-dir").c_str());
}
...
BOOST_AUTO_TEST_CASE(testRejectsTooDeepNestingUnderChildId) {
    ...
    BOOST_TEST_REQUIRE(result.s_Ok == false);
    BOOST_REQUIRE_EQUAL(result.s_Rejected.size(), 1);
    BOOST_REQUIRE(result.s_Rejected[0].s_Reason == ml::sandbox::EChildIpcPathRejection::E_WrongDepth);
...
BOOST_AUTO_TEST_CASE(testRejectsDuplicateLiteralArgument) {
    ...
    const ml::sandbox::SChildIpcValidationResult result{
        ml::sandbox::validateChildIpcLaunchSpec(fixture.canonicalTrustedBase(), {arg, arg})};

    BOOST_TEST_REQUIRE(result.s_Ok == false);
    BOOST_REQUIRE_EQUAL(result.s_Rejected.size(), 1);
    BOOST_REQUIRE(result.s_Rejected[0].s_Reason == ml::sandbox::EChildIpcPathRejection::E_Duplicate);
}
...
BOOST_AUTO_TEST_CASE(testRejectsChildIdMismatchAcrossOptions) {
    ...
    CTempChildIpcFixture fixtureB{"child-10b"};

    const ml::sandbox::SChildIpcValidationResult result{ml::sandbox::validateChildIpcLaunchSpec(
        fixtureA.canonicalTrustedBase(),
        {"--input=" + fixtureA.childRoot() + "/input.fifo",
         "--output=" + fixtureB.childRoot() + "/output.fifo"})};

    BOOST_TEST_REQUIRE(result.s_Ok == false);
    BOOST_REQUIRE_EQUAL(result.s_Rejected.size(), 1);
    BOOST_REQUIRE(result.s_Rejected[0].s_Reason == ml::sandbox::EChildIpcPathRejection::E_ChildIdMismatch);
}
...
BOOST_AUTO_TEST_CASE(testIgnoresScalarOptionsAsCandidatePaths) {
    ...
    const ml::sandbox::SChildIpcValidationResult result{ml::sandbox::validateChildIpcLaunchSpec(
        ...
        {"--input=" + fixture.childRoot() + "/input.fifo", "--modelId=../../../etc/passwd",
         "--inputIsPipe"})};
