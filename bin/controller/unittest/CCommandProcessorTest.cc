...
BOOST_AUTO_TEST_CASE(testStartFailsWithSettingHintWhenHostCannotConfine) {
    ...
    std::ostringstream responseStream;
    {
        ...
        BOOST_REQUIRE_EQUAL(false, processor.handleCommand(startCommand(
                                       32, PROCESS_PATH,
                                       copyArgs(TARGET_FILE, {"--requireSandbox"}))));
    }
    ...
    BOOST_TEST_REQUIRE(response.find("xpack.ml.trained_models.sandbox_enabled") != std::string::npos);
}
