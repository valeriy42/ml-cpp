...
BOOST_AUTO_TEST_CASE(testSandbox2RouteDegradesToLandlockRungWithInjectedConfinement) {
    ...
    ml::controller::CProcessSpawnerRouter::TStrVec sandboxedPaths{PROCESS_PATH};
    ml::controller::CProcessSpawnerRouter router{permittedPaths, sandboxedPaths,
                                                 [injectedHost] { return injectedHost; }};
    ...
    ifs.close();
    std::remove(outputFile.c_str());
    BOOST_REQUIRE_EQUAL(ml::controller::CProcessSpawnerRouter::RESTRICT_FILESYSTEM_TOKEN, firstLine);
...
BOOST_AUTO_TEST_CASE(testSandbox2RouteFailsClosedWithInjectedUnavailableConfinement) {
    ...
    BOOST_TEST_REQUIRE(router.lastSpawnFailureReason().find(
                           "xpack.ml.trained_models.sandbox_enabled") != std::string::npos);
    BOOST_TEST_REQUIRE(router.lastSpawnFailureReason().find("deactivate") != std::string::npos);
...
BOOST_AUTO_TEST_CASE(testLastSpawnFailureReasonClearedByALaterSuccessfulSpawn) {
    ...
    std::remove(outputFile.c_str());
    ml::controller::CProcessSpawnerRouter::TStrVec okArgs{SHELL_FLAG, copyArgsScript(outputFile)};
